...
import type { Wreck, WreckCompactItem, WreckFeature } from "@/lib/domain/wreck";
...
export const compactWrecksFor = (era?: string): WreckCompactItem[] => demoWrecks.filter((wreck) => {
  if (!era || era === "all") return true;
  if (!wreck.sunkYear) return false;
  return era === "before-1900" ? wreck.sunkYear < 1900 : era === "1900-1945" ? wreck.sunkYear >= 1900 && wreck.sunkYear <= 1945 : wreck.sunkYear > 1945;
}).map(({ id, name, category, type, coordinates, sunkYear, depthM }) => ({ id, name, category, type, coordinates, sunkYear, depthM }));
